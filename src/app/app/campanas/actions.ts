...
export async function createReservationAction(
  ...
  if (!user) {
    ...
    const next = slugStr
      ? `/app/campanas/${encodeURIComponent(slugStr)}?reservar=${quantity}`
      : "/perfil";
  ...
  await sendEmail({
    ...
    text: reservationConfirmedEmail({
      ...
      quantity,
      depositCents,
      campaignUrl: `${origin}/app/campanas/${campaign.slug}`,
    }),
  });
  ...
  redirect(`/app/campanas/${campaign.slug}/reservada?quantity=${quantity}`);
}
